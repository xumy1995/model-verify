...
| 模型 | 数据集 | 评测项 | cuda指标值 | mx-c500指标值 | 对比结果 |
|---|---|---|---:|---:|---:|
| ResNet-50 | ImageNet-1k val | 每样本推理时间 (ms/sample) | 0.3503 | 0.3552 | 98.63% |
| ResNet-50 | ImageNet-1k val | 每样本端到端时间 (ms/sample) | 0.3814 | 0.3717 | 102.61% |
| ViT-B/16 | ImageNet-1k val | 每样本推理时间 (ms/sample) | 0.5278 | 1.0554 | **50.01%** |
| ViT-B/16 | ImageNet-1k val | 每样本端到端时间 (ms/sample) | 0.5617 | 1.0725 | **52.38%** |
| CLIP ViT-B/32 (zero-shot) | ImageNet-1k val | 每样本推理时间 (ms/sample) | 0.2218 | 0.2921 | **75.93%** |
| CLIP ViT-B/32 (zero-shot) | ImageNet-1k val | 每样本端到端时间 (ms/sample) | 0.2540 | 0.3089 | 82.23% |
| DETR-ResNet-50 | COCO val | 每样本推理时间 (ms/sample) | 13.694 | 18.111 | **75.61%** |
| DETR-ResNet-50 | COCO val | 每样本端到端时间 (ms/sample) | 14.802 | 18.783 | **78.80%** |
| YOLO26n | COCO val | 每样本推理时间 (ms/image) | 0.9823 | 0.9189 | 106.90% |
| YOLO26n | COCO val | 每样本端到端时间 (ms/image) | 1.4070 | 1.3309 | 105.72% |
| YOLO26s | COCO val | 每样本推理时间 (ms/image) | 1.3757 | 1.5145 | 90.84% |
| YOLO26s | COCO val | 每样本端到端时间 (ms/image) | 1.7983 | 1.9276 | 93.29% |
| YOLO26m | COCO val | 每样本推理时间 (ms/image) | 2.1111 | 2.6931 | **78.39%** |
| YOLO26m | COCO val | 每样本端到端时间 (ms/image) | 2.5288 | 3.1363 | 80.63% |
| YOLO26l | COCO val | 每样本推理时间 (ms/image) | 2.5991 | 3.4495 | **75.35%** |
| YOLO26l | COCO val | 每样本端到端时间 (ms/image) | 3.0187 | 3.9130 | **77.15%** |
| YOLO26x | COCO val | 每样本推理时间 (ms/image) | 4.2067 | 5.7145 | **73.61%** |
| YOLO26x | COCO val | 每样本端到端时间 (ms/image) | 4.6206 | 6.1489 | **75.15%** |
