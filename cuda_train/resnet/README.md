...
本目录用于验证 ResNet-50 在 CUDA 上的训练流程，并对齐 TorchVision 官方 ImageNet-1K 基线。

## 环境

    cd /data/xumengying/model-verify/cuda_train
...
当前训练脚本已经适配 Kaggle 下载的 ILSVRC 目录，不需要转换成 ImageFolder，也不需要重新解压：
    ...
    /data/xumengying/models_and_datasets/ILSVRC/
...
确认数据完整性：
    ...
    find /data/xumengying/models_and_datasets/ILSVRC/Data/CLS-LOC/train -mindepth 1 -maxdepth 1 -type d | wc -l
    find /data/xumengying/models_and_datasets/ILSVRC/Data/CLS-LOC/train -type f | wc -l
    find /data/xumengying/models_and_datasets/ILSVRC/Data/CLS-LOC/val -type f | wc -l
    wc -l /data/xumengying/models_and_datasets/ILSVRC/ImageSets/CLS-LOC/val.txt
...
多卡使用 torchrun 启动；batch-size 是每张卡的 batch size：
    ...
    torchrun --standalone --nproc_per_node=8 resnet/train_resnet50.py --recipe v2 --data /data/xumengying/models_and_datasets/ILSVRC --output-dir resnet/checkpoints/resnet50_v2_ddp --log-file resnet/logs/resnet50_v2_ddp.log --batch-size 128 --workers 8 --log-interval 100
