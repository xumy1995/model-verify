...
DEFAULT_MODEL_PATH = "/data/xumengying/models_and_datasets/resnet-50"
DEFAULT_DATASET_PATH = "/data/xumengying/models_and_datasets/imagenet-1k"
DEFAULT_CACHE_DIR = "/data/xumengying/hf_datasets_cache"
...
def load_imagenet_dataset(dataset_path, split, cache_dir):
    ...
    try:
        if split != "auto":
            # The local ImageNet snapshot contains train and test parquet files.
            # Its dataset script may inspect/build every split even when a
            # single split is requested.  Point the parquet builder directly at
            # the requested files so train is never discovered or generated.
        ...
        # Probe evaluation splits one at a time.  Loading the dataset without a
        # split can materialize every available split (including ``train``),
        # which is unnecessary for evaluation and may require a large download.
        for candidate in ("validation", "val", "test"):
            parquet_pattern = os.path.join(dataset_path, "data", f"{candidate}-*.parquet")
            parquet_files = sorted(glob.glob(parquet_pattern))
            if parquet_files:
            ...
            try:
                ...
                continue
        # Preserve the original error/diagnostic for datasets with no standard
        # evaluation split.
...
def collate_images(processor):
    def collate(batch):
        ...
        labels = torch.tensor([item.get("label", -1) for item in batch], dtype=torch.long)
...
def run_eval(args, split_name, dataset, processor, device_name):
    ...
    avg_latency_ms = 1000.0 * inference_time / measured_seen if measured_seen > 0 else 0.0
